...
func (d *Daemon) dumpOldIPCache() (localPrefixes map[netip.Prefix]identity.NumericIdentity, err error) {
...
func (d *Daemon) restoreIPCache(localPrefixes map[netip.Prefix]identity.NumericIdentity, restoredIdentities map[identity.NumericIdentity]*identity.Identity) {
	...
	for prefix, nid := range localPrefixes {
		...
		if id == nil {
			...
			// Check to see if any other prefixes share this identity. If not, then
			// add CIDR labels as well.
			found := false
			for tryPrefix, tryNID := range localPrefixes {
				if tryNID == nid && tryPrefix != prefix {
					found = true
					break
				}
			}
