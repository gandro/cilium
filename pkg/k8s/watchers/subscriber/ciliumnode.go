...
type CiliumNode interface {
	OnAddCiliumNode(node *ciliumv2.CiliumNode) error
	OnUpdateCiliumNode(oldObj, newObj *ciliumv2.CiliumNode) error
	OnDeleteCiliumNode(node *ciliumv2.CiliumNode) error
}
...
func (l *CiliumNodeChain) OnAddCiliumNode(node *ciliumv2.CiliumNode) error {
	l.RLock()
	defer l.RUnlock()
	errs := []error{}
	for _, s := range l.subs {
		if err := s.OnAddCiliumNode(node); err != nil {
...
func (l *CiliumNodeChain) OnUpdateCiliumNode(oldNode, newNode *ciliumv2.CiliumNode) error {
	l.RLock()
	defer l.RUnlock()
	errs := []error{}
	for _, s := range l.subs {
		if err := s.OnUpdateCiliumNode(oldNode, newNode); err != nil {
			errs = append(errs, err)
		}
	}
	if len(errs) > 0 {
...
func (l *CiliumNodeChain) OnDeleteCiliumNode(node *ciliumv2.CiliumNode) error {
	l.RLock()
	defer l.RUnlock()
	errs := []error{}
	for _, s := range l.subs {
		if err := s.OnDeleteCiliumNode(node); err != nil {
			errs = append(errs, err)
		}
	}
	if len(errs) > 0 {
